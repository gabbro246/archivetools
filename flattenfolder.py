...
def flatten_folder(root_folder, rename_files, depth=None):
    if not os.path.isdir(root_folder):
        logging.error("The specified path is not a directory.", extra={'target': root_folder})
        return
    ...
    while True:
        ...
        if current_file_count == prev_file_count:
            logging.warning("No progress detected, stopping to avoid an infinite loop.", extra={'target': root_folder})
            break
        prev_file_count = current_file_count

        for subdir in os.listdir(root_folder):
            subdir_path = os.path.join(root_folder, subdir)
            relative_subdir_path = os.path.relpath(subdir_path, root_folder)
            if os.path.isdir(subdir_path):
                any_folder_found = True
                for item in os.listdir(subdir_path):
                    item_path = os.path.join(subdir_path, item)
                    target_path = os.path.join(root_folder, item)
                    relative_item_path = os.path.relpath(item_path, root_folder)
                    ...
                    if os.path.exists(target_path):
                        if rename_files:
                            ...
                            logging.info("Renaming to avoid conflict.", extra={'target': relative_item_path})
                        else:
                            logging.info("Skipping due to naming conflict.", extra={'target': relative_item_path})
                            continue

                    try:
                        shutil.move(item_path, target_path)
                        logging.info("Moved file.", extra={'target': relative_item_path})
                    except Exception as e:
                        logging.error(f"Error moving file: {e}", extra={'target': relative_item_path})

                if not os.listdir(subdir_path):
                    try:
                        os.rmdir(subdir_path)
                        logging.info("Removed empty folder.", extra={'target': relative_subdir_path})
                    except Exception as e:
                        logging.error(f"Could not remove folder: {e}", extra={'target': relative_subdir_path})
                else:
                    logging.warning("Skipping deletion: folder is not empty.", extra={'target': relative_subdir_path})
